...
@Component({
  ...
  styleUrl: './agent-pipeline.component.scss',
})
export class AgentPipelineComponent {
  ...
  pipelinePhase = computed<'idle' | 'core-engine' | 'agents' | 'decision-engine'>(() => {
    ...
    if (step.startsWith('Orchestrator Agent') || step.startsWith('Connecting')) {
      return 'core-engine';
    }
    if (agents.length > 0 && agents.some(a => a.status === 'running')) {
      return 'agents';
    }
    if (step.includes('Decision Engine') || step.startsWith('Generating MoSCoW')) {
